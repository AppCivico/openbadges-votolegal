...
my $org = {
  ...
  "name" => "RAPS",
  "image" =>
"https://raw.githubusercontent.com/AppCivico/openbadges-votolegal/master/logo-aplicacao-branco-RAPS.jpg",
...
if ( @pref > 10 ) {
  ...
  write_file(
    "openbadges/organization.json",
    { binmode => ':raw' },
    to_json( $org, { utf8 => 1, pretty => 1 } )
  );
  write_file(
    "openbadges/raps.json",
    { binmode => ':raw' },
    to_json( $badge_raps, { utf8 => 1, pretty => 1 } )
  );
  ...
  write_file(
    ...
    to_json(
      {
        badges => [
          map { +{ url => $_->{verify}{url}, nome => $_->{'raps:nome'} } } @pref
        ]
      },
      { utf8 => 1, pretty => 1 }
    )
